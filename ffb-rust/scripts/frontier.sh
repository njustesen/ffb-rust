...
#
# MATCHUP=<race> (default amazon) selects the mirror matchup (passed through to
# first_state_divergence.sh); FRONTIER_LOG names the gate log to take reds from.
#
...
if [ $# -eq 0 ]; then
  ...
  LOG=${FRONTIER_LOG:-$(ls -t $S/iter*_${E}_amz.log 2>/dev/null | head -1)}   # amazon-era default
  [ -z "$LOG" ] && { echo "no gate log for $E; pass seeds explicitly"; exit 1; }
  echo "reds from $LOG"
  set -- $(grep -oE "^PARITY FAIL seed=[0-9]+" "$LOG" | grep -oE "[0-9]+$")
fi
echo "=== $E ${MATCHUP:-amazon} frontier: $# seeds ==="
for s in "$@"; do
