...
## Current Status (2026-07-11, Phase AAA done — typed replayable command log wired into `ffb-server::GameState`; 7/11 server handler gaps closed, 4 remain (replay engine + UploadGame HTTP backup))
...
**Translation progress (honest, by Java LOC): ~100% of in-scope (~235.2k LOC); ~85% of everything (279k).** The common + server layers are genuinely done except 4 `~` `ffb-server` handler rows (down from 6 at the start of this session, 8 before Phase ZY, 11 before Phase ZX, 35 before Phase ZW.1). `ffb-client-logic`: all 373 in-scope files are genuinely `✓` (unchanged this session — Phase ZZ touched only `ffb-model`/`ffb-engine`/`ffb-server`). Remaining `—` rows (271 files: Swing dialog/ui/layer/animation/overlay/sound) stay permanently skipped per the ZW plan's triage.
