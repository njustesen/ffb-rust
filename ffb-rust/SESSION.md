...
## Current Status (2026-07-11, Phase AAB done — replay playback engine wired for real; 9/11 server handler gaps closed, 2 remain (`ServerCommandHandlerReplay`'s DB-fallback enqueue, blocked on Phase AAC's `ServerRequestLoadReplay` trait fix; `UploadGame` HTTP backup))
...
**Translation progress (honest, by Java LOC): ~100% of in-scope (~235.2k LOC); ~85% of everything (279k).** The common + server layers are genuinely done except 2 `~` `ffb-server`-side rows (`ServerCommandHandlerReplay`'s DB-fallback branch, `ServerCommandHandlerUploadGame` — down from 4 after Phase AAB, 6 before Phase AAA, 8 before Phase ZY, 11 before Phase ZX, 35 before Phase ZW.1). `ffb-client-logic`: all 373 in-scope files are genuinely `✓` (unchanged this session — Phase ZZ touched only `ffb-model`/`ffb-engine`/`ffb-server`). Remaining `—` rows (271 files: Swing dialog/ui/layer/animation/overlay/sound) stay permanently skipped per the ZW plan's triage.

**Tests:** 17,238 passing, 0 failing (down from 17,399 at the start of this session — the bundled cleanup deleted more orphaned-stub tests, across 59 dead files, than the phase's own ~20 new/updated tests added back).
