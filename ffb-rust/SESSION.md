...
## Current Status (2026-07-11, Phases AAB + AAC done — replay playback engine wired for real, UploadGame's missing-game/HTTP-backup branch closed; 10/11 originally-deferred server handler gaps now closed, 1 remains: `ServerCommandHandlerReplay`'s DB-fallback enqueue, which was blocked on Phase AAC's `ServerRequestLoadReplay` trait fix — that fix landed this session, so this last piece is now unblocked-in-principle and just needs the final wiring pass)
...
**Translation progress (honest, by Java LOC): ~100% of in-scope (~235.2k LOC); ~85% of everything (279k).** The common + server layers are genuinely done except 1 `~` `ffb-server` row (`ServerCommandHandlerReplay`'s DB-fallback branch — down from 2 after Phase AAC landed, 4 before Phase AAB, 6 before Phase AAA, 8 before Phase ZY, 11 before Phase ZX, 35 before Phase ZW.1) plus `ffb-engine`'s `UtilServerHttpClient` (corrected from a stale `✓` to `~` this session — deliberately unimplemented, see Phase AAC below). `ffb-client-logic`: all 373 in-scope files are genuinely `✓` (unchanged this session). Remaining `—` rows (271 files: Swing dialog/ui/layer/animation/overlay/sound) stay permanently skipped per the ZW plan's triage.

**Tests:** merged total pending final `cargo test --workspace` re-run this session (Phase AAB alone: 17,249 → 17,265, +16; Phase AAC alone: 17,249 → 17,259, +10 — both branched from the same Phase AAA base, so the combined total after merge is not a simple sum and is re-verified below).
