...
foreach ($race in $races) {
    $r = & .\target\release\ffb-parity.exe --home $race --away $race --edition bb2025 --seeds 10 --no-abort 2>&1 | Select-String 'PARITY:'
    $line = "$race`: $r"
    Write-Host $line
    if ($r -match '10/10') { $pass++ } else { $fail++; $failRaces += $race }
}
Write-Host "---"
Write-Host "Total: $pass/25 races 10/10, $fail failed"
