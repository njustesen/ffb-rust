...
use ffb_model::report::bb2016::report_swoop_player::ReportSwoopPlayer;
...
impl ReportMessage for SwoopPlayerMessage {
    ...
    fn render(&self, status_report: &mut StatusReport, _game: &Game, report: &Self::Report) {
        ...
        if !rolls.is_empty() {
            ...
            let mut status = if rolls.len() > 1 { String::from("Swoop Rolls [ ") } else { String::from("Swoop Roll [ ") };
            ...
            let status = format!(
                ...
                report.get_start_coordinate().x, report.get_start_coordinate().y,
                report.get_end_coordinate().x, report.get_end_coordinate().y
            );
...
mod tests {
    ...
    #[test]
    fn get_key_is_swoop_player() {
        assert_eq!(SwoopPlayerMessage.get_key(), "swoopPlayer");
    }

    #[test]
    fn reports_swoop_and_movement() {
        let mut status_report = StatusReport::new();
        let game = make_game();
        let report = ReportSwoopPlayer::new(FieldCoordinate::new(1, 2), FieldCoordinate::new(3, 4), vec![Direction::West], vec![4]);
        SwoopPlayerMessage.render(&mut status_report, &game, &report);
        assert_eq!(status_report.rendered_runs[0].text.as_deref(), Some("Swoop Roll [ 4 ] West"));
        assert_eq!(status_report.rendered_runs[2].text.as_deref(), Some("Player swoops from square (1,2) to square (3,4)."));
    }

    #[test]
    fn empty_rolls_produce_no_output() {
        let mut status_report = StatusReport::new();
        let game = make_game();
        let report = ReportSwoopPlayer::new(FieldCoordinate::new(0, 0), FieldCoordinate::new(0, 0), vec![], vec![]);
        SwoopPlayerMessage.render(&mut status_report, &game, &report);
        assert!(status_report.rendered_runs.is_empty());
    }

    #[test]
    fn multiple_rolls_use_plural_label() {
        let mut status_report = StatusReport::new();
        let game = make_game();
        let report = ReportSwoopPlayer::new(FieldCoordinate::new(1, 1), FieldCoordinate::new(2, 2), vec![Direction::East, Direction::South], vec![3, 5]);
        SwoopPlayerMessage.render(&mut status_report, &game, &report);
        assert_eq!(status_report.rendered_runs[0].text.as_deref(), Some("Swoop Rolls [ 3, 5 ] East, South"));
    }
}
