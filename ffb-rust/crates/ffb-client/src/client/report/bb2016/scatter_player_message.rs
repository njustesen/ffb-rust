use crate::client::report::report_message_base::{map_to_local, ReportMessage};
use crate::client::status_report::StatusReport;
use crate::client::text_style::TextStyle;
use ffb_model::model::game::Game;
use ffb_model::report::report_id::ReportId;
use ffb_model::report::report_scatter_player::ReportScatterPlayer;

pub struct ScatterPlayerMessage;

impl ReportMessage for ScatterPlayerMessage {
    type Report = ReportScatterPlayer;

    fn report_id(&self) -> ReportId {
        ReportId::SCATTER_PLAYER
    }

    fn render(&self, status_report: &mut StatusReport, _game: &Game, report: &Self::Report) {
        let rolls = report.get_rolls();
        if !rolls.is_empty() {
            let indent = status_report.get_indent();
            let mut status = if rolls.len() > 1 { String::from("Scatter Rolls [ ") } else { String::from("Scatter Roll [ ") };
            for (i, roll) in rolls.iter().enumerate() {
                if i > 0 {
                    status.push_str(", ");
                }
                status.push_str(&roll.to_string());
            }
            status.push_str(" ] ");
            let directions = report.get_directions();
            for (i, direction) in directions.iter().enumerate() {
                if i > 0 {
                    status.push_str(", ");
                }
                status.push_str(map_to_local(*direction).name());
            }
            status_report.println_indent_style(indent, TextStyle::ROLL, &status);
            let status = format!(
                "Player scatters from square ({},{}) to square ({},{}).",
                report.get_start_coordinate().x, report.get_start_coordinate().y,
                report.get_end_coordinate().x, report.get_end_coordinate().y
            );
            status_report.println_indent(indent + 1, &status);
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use ffb_model::enums::{Direction, Rules};
    use ffb_model::model::team::Team;
    use ffb_model::types::FieldCoordinate;

    fn make_team(id: &str) -> Team {
        Team {
            id: id.into(), name: format!("Team {id}"), race: "Human".into(), roster_id: "human".into(), coach: "Coach".into(),
            rerolls: 0, apothecaries: 0, bribes: 0, master_chefs: 0, prayers_to_nuffle: 0, bloodweiser_kegs: 0, riotous_rookies: 0,
            cheerleaders: 0, assistant_coaches: 0, fan_factor: 0, dedicated_fans: 0, team_value: 0, treasury: 0,
            special_rules: vec![], players: vec![], vampire_lord: false, necromancer: false,
        }
    }

    fn make_game() -> Game {
        Game::new(make_team("home"), make_team("away"), Rules::Bb2016)
    }

    #[test]
    fn get_key_is_scatter_player() {
        assert_eq!(ScatterPlayerMessage.get_key(), "scatterPlayer");
    }

    #[test]
    fn reports_scatter_and_movement() {
        let mut status_report = StatusReport::new();
        let game = make_game();
        let report = ReportScatterPlayer::new(
            FieldCoordinate::new(3, 4), FieldCoordinate::new(5, 6), vec![Direction::North], vec![2], Some(true),
        );
        ScatterPlayerMessage.render(&mut status_report, &game, &report);
        assert_eq!(status_report.rendered_runs[0].text.as_deref(), Some("Scatter Roll [ 2 ] North"));
        assert_eq!(status_report.rendered_runs[2].text.as_deref(), Some("Player scatters from square (3,4) to square (5,6)."));
    }

    #[test]
    fn empty_rolls_produce_no_output() {
        let mut status_report = StatusReport::new();
        let game = make_game();
        let report = ReportScatterPlayer::new(FieldCoordinate::new(0, 0), FieldCoordinate::new(0, 0), vec![], vec![], None);
        ScatterPlayerMessage.render(&mut status_report, &game, &report);
        assert!(status_report.rendered_runs.is_empty());
    }

    #[test]
    fn multiple_rolls_use_plural_label() {
        let mut status_report = StatusReport::new();
        let game = make_game();
        let report = ReportScatterPlayer::new(
            FieldCoordinate::new(1, 1), FieldCoordinate::new(2, 2), vec![Direction::East, Direction::South], vec![3, 5], Some(false),
        );
        ScatterPlayerMessage.render(&mut status_report, &game, &report);
        assert_eq!(status_report.rendered_runs[0].text.as_deref(), Some("Scatter Rolls [ 3, 5 ] East, South"));
    }
}
