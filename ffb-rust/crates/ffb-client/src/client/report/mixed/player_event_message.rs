use crate::client::report::report_message_base::{print_player, ReportMessage};
use crate::client::status_report::StatusReport;
use crate::client::text_style::TextStyle;
use ffb_model::model::game::Game;
use ffb_model::report::mixed::report_player_event::ReportPlayerEvent;
use ffb_model::report::report_id::ReportId;

/// 1:1 translation of `PlayerEventMessage.java`.
pub struct PlayerEventMessage;

impl ReportMessage for PlayerEventMessage {
    type Report = ReportPlayerEvent;

    fn report_id(&self) -> ReportId {
        ReportId::PLAYER_EVENT
    }

    fn render(&self, status_report: &mut StatusReport, game: &Game, report: &Self::Report) {
        let player = report.get_player_id().and_then(|id| game.player(id));
        let indent = status_report.get_indent() + 1;
        print_player(status_report, game, indent, false, player);
        status_report.println_indent_style(indent, TextStyle::NONE, &format!(" {}", report.get_event_message().unwrap_or_default()));
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use ffb_model::enums::Rules;
    use ffb_model::model::player::Player;
    use ffb_model::model::team::Team;

    fn make_team(id: &str, players: Vec<Player>) -> Team {
        Team {
            id: id.into(),
            name: format!("Team {id}"),
            race: "Human".into(),
            roster_id: "human".into(),
            coach: format!("Coach {id}"),
            rerolls: 0,
            apothecaries: 0,
            bribes: 0,
            master_chefs: 0,
            prayers_to_nuffle: 0,
            bloodweiser_kegs: 0,
            riotous_rookies: 0,
            cheerleaders: 0,
            assistant_coaches: 0,
            fan_factor: 0,
            dedicated_fans: 0,
            team_value: 0,
            treasury: 0,
            special_rules: vec![],
            players,
            vampire_lord: false,
            necromancer: false,
        }
    }

    fn make_game() -> Game {
        let player = Player { id: "p1".into(), name: "Grobnik".into(), ..Player::default() };
        Game::new(make_team("home", vec![player]), make_team("away", vec![]), Rules::Bb2020)
    }

    #[test]
    fn renders_player_and_event_message() {
        let game = make_game();
        let report = ReportPlayerEvent::new(Some("p1".into()), Some("gets a niggling injury.".into()));
        let mut status_report = StatusReport::new();
        PlayerEventMessage.render(&mut status_report, &game, &report);
        assert_eq!(status_report.rendered_runs[0].text.as_deref(), Some("Grobnik"));
        assert_eq!(status_report.rendered_runs[1].text.as_deref(), Some(" gets a niggling injury."));
    }

    #[test]
    fn indent_is_offset_by_one() {
        let game = make_game();
        let mut status_report = StatusReport::new();
        status_report.set_indent(1);
        let report = ReportPlayerEvent::new(Some("p1".into()), Some("event".into()));
        PlayerEventMessage.render(&mut status_report, &game, &report);
        use crate::client::paragraph_style::ParagraphStyle;
        assert_eq!(status_report.rendered_runs[0].paragraph_style, Some(ParagraphStyle::INDENT_2));
    }

    #[test]
    fn missing_event_message_renders_empty() {
        let game = make_game();
        let report = ReportPlayerEvent::new(Some("p1".into()), None);
        let mut status_report = StatusReport::new();
        PlayerEventMessage.render(&mut status_report, &game, &report);
        assert_eq!(status_report.rendered_runs[1].text.as_deref(), Some(" "));
    }

    #[test]
    fn report_id_and_key() {
        assert_eq!(PlayerEventMessage.report_id(), ReportId::PLAYER_EVENT);
        assert_eq!(PlayerEventMessage.get_key(), "playerEvent");
    }
}
