/// 1:1 translation of com.fumbbl.ffb.server.handler.talk.TalkHandlerProneLive.
/// Live variant of TalkHandlerProne — uses DecoratingCommandAdapter, SPEC client, EDIT_STATE privilege.
...
pub struct TalkHandlerProneLive {
    base: TalkHandlerProne,
}

impl TalkHandlerProneLive {
    /// Java: `TalkHandlerProneLive()`.
    pub fn new() -> Self {
        let adapter = DecoratingCommandAdapter::new();
        let mut privileges = HashSet::new();
        privileges.insert(Privilege::EditState);
        Self {
            base: TalkHandlerProne::new(&adapter, Client::Spec, Environment::None, privileges),
        }
    }

    pub fn base(&self) -> &TalkHandler { self.base.base() }

    /// Java: `handle` — delegates to TalkHandlerProne with live game settings.
...
impl Default for TalkHandlerProneLive {
...
mod tests {
    ...
    fn handle_delegates_to_base_logic() {
        ...
        let commands = vec!["/prone_home".to_string(), "1".to_string()];
