/// 1:1 translation of com.fumbbl.ffb.server.handler.talk.TalkHandlerMovePlayerTest.
/// Test variant of TalkHandlerMovePlayer — uses IdentityCommandAdapter, PLAYER client, TEST_GAME env.
...
pub struct TalkHandlerMovePlayerTest {
    base: TalkHandlerMovePlayer,
}

impl TalkHandlerMovePlayerTest {
    /// Java: `TalkHandlerMovePlayerTest()`.
    pub fn new() -> Self {
        let adapter = IdentityCommandAdapter::new();
        Self {
            base: TalkHandlerMovePlayer::new(&adapter, Client::Player, Environment::TestGame, Default::default()),
        }
    }

    pub fn base(&self) -> &TalkHandler { self.base.base() }

    /// Java: `handle` — delegates to TalkHandlerMovePlayer with test game settings.
    pub fn handle(
        &self,
...
impl Default for TalkHandlerMovePlayerTest {
...
mod tests {
    ...
    #[test]
    fn construct() { let _ = TalkHandlerMovePlayerTest::new(); }

    #[test]
    fn handle_delegates_to_base_logic() {
        let h = TalkHandlerMovePlayerTest::new();
        ...
        let commands = vec!["/move_player".to_string(), "1".to_string(), "North".to_string(), "3".to_string()];
        let info = h.handle(&mut fm, &commands, &team, 100, 1, &sm);
        assert_eq!(fm.player_coordinate("p1"), Some(FieldCoordinate::new(5, 2)));
