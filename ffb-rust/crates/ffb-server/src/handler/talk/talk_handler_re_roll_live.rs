/// 1:1 translation of com.fumbbl.ffb.server.handler.talk.TalkHandlerReRollLive.
/// Live variant of TalkHandlerReRoll — uses IdentityCommandAdapter, SPEC client, EDIT_STATE privilege.
...
mod tests {
    ...
    #[test]
    fn construct() { let _ = TalkHandlerReRollLive::new(); }

    #[test]
    fn handle_delegates_to_base_logic() {
        let h = TalkHandlerReRollLive::new();
        let mut game = Game::new(empty_team("home"), empty_team("away"), Rules::Bb2025);
        let team = game.team_home.clone();
        let commands = vec!["/set_rerolls".to_string(), "4".to_string()];
        h.handle(&mut game, &commands, &team);
        assert_eq!(game.turn_data_home.rerolls, 4);
    }
}
