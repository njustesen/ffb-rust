/// 1:1 translation of com.fumbbl.ffb.server.handler.talk.TalkHandlerProneTest.
/// Test variant of TalkHandlerProne — uses IdentityCommandAdapter, PLAYER client, TEST_GAME env.
...
use crate::handler::talk::talk_handler_prone::TalkHandlerProne;
use crate::handler::talk::talk_requirements::{Client, Environment};

pub struct TalkHandlerProneTest {
    base: TalkHandlerProne,
}

impl TalkHandlerProneTest {
    /// Java: `TalkHandlerProneTest()`.
    pub fn new() -> Self {
        let adapter = IdentityCommandAdapter::new();
        Self {
            base: TalkHandlerProne::new(&adapter, Client::Player, Environment::TestGame, Default::default()),
        }
    }

    pub fn base(&self) -> &TalkHandler { self.base.base() }

    /// Java: `handle` — delegates to TalkHandlerProne with test game settings.
...
impl Default for TalkHandlerProneTest {
...
mod tests {
    ...
    #[test]
    fn construct() { let _ = TalkHandlerProneTest::new(); }

    #[test]
    fn handle_delegates_to_base_logic() {
        let h = TalkHandlerProneTest::new();
