/// 1:1 translation of com.fumbbl.ffb.server.handler.talk.TalkHandlerStandup.
/// Abstract handler for `/standup` command — stands up prone/stunned field players.
...
pub struct TalkHandlerStandup {
    pub required_client: Client,
    pub required_environment: Environment,
    pub requires_one_privilege_of: Vec<Privilege>,
}

impl TalkHandlerStandup {
    /// Java: `super("/standup", 1, ...)`.
    pub const COMMAND: &'static str = "/standup";
    ...
    /// dugout box is set to STANDING and marked active.
    pub fn handle(&self, game: &mut Game, team: &Team, commands: &[String]) -> Vec<String> {
        ...
        for id in find_player_ids_in_command(team, commands, Self::COMMAND_PARTS_THRESHOLD) {
            ...
            messages.push(format!("Player {name} stands up."));
        }
        messages
    }
}

impl Default for TalkHandlerStandup {
    fn default() -> Self {
...
mod tests {
    ...
    fn game_with_prone_player() -> (Game, Team) {
        ...
        g.field_model.set_player_state("p1", PlayerState::new(PS_PRONE));
        (g, team)
    }
    ...
    fn handle_stands_up_prone_player_on_pitch() {
        ...
        let commands = vec!["/standup".into(), "1".into()];
        let messages = h.handle(&mut g, &team, &commands);
        assert_eq!(messages, vec!["Player Alice stands up.".to_string()]);
        assert_eq!(g.field_model.player_state("p1").unwrap().base(), PS_STANDING);
    }
    ...
    fn handle_skips_player_in_box() {
        ...
        let commands = vec!["/standup".into(), "all".into()];
        let messages = h.handle(&mut g, &team, &commands);
