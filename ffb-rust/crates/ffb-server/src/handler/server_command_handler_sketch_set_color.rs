/// 1:1 translation of com.fumbbl.ffb.server.handler.ServerCommandHandlerSketchSetColor.
///
/// Java: `extends AbstractServerCommandHandlerSketch<ClientCommandSketchSetColor, ServerCommandSketchSetColor>`.
...
pub struct ServerCommandHandlerSketchSetColor {
...
impl ServerCommandHandlerSketchSetColor {
    pub fn new(
        sketch_manager: Arc<Mutex<ServerSketchManager>>,
        replay_session_manager: Arc<Mutex<ReplaySessionManager>>,
    ) -> Self {
    ...
    /// Java: `getId()` — returns `NetCommandId.CLIENT_SKETCH_SET_COLOR`.
    pub fn get_id(&self) -> NetCommandId {
        NetCommandId::ClientSketchSetColor
    }

    /// Java: `updateSketchManager(Session, ClientCommandSketchSetColor)` —
    /// `command.getSketchIds().forEach(id -> sketchManager.setRgb(session, id, command.getRbg()))`.
    pub fn update_sketch_manager(
        &self,
        session_id: SessionId,
        command: &ClientCommandSketchSetColor,
    ) {
        let mut mgr = self.sketch_manager.lock().unwrap();
        for id in command.get_sketch_ids() {
            mgr.set_rgb(&session_id.to_string(), id, command.get_rgb());
        }
    }

    /// Java: `createServerCommand(String, ClientCommandSketchSetColor)` —
    /// `new ServerCommandSketchSetColor(coach, command.getSketchIds(), command.getRbg())`.
    pub fn create_server_command(
        &self,
        coach: &str,
        command: &ClientCommandSketchSetColor,
    ) -> ServerCommandSketchSetColor {
        ServerCommandSketchSetColor::new(coach, command.get_sketch_ids().to_vec(), command.get_rgb())
    }

    /// Java: `getServer().getCommunication().sendToReplaySession(otherSession, serverCommand)`'s
    /// wire payload. `ServerCommandSketchSetColor` has no serde impl of its own (it isn't a
    ...
    fn to_json(command: &ServerCommandSketchSetColor) -> String {
        format!(
            "{{\"netCommandId\":\"{}\",\"coach\":{:?},\"sketchIds\":{},\"rbg\":{}}}",
            NetCommandId::ServerSketchSetColor.name(),
            command.get_coach(),
...
impl Default for ServerCommandHandlerSketchSetColor {
    fn default() -> Self {
        Self::new(
...
mod tests {
    ...
    #[test]
    fn update_sketch_manager_sets_rgb_for_each_id() {
        let handler = ServerCommandHandlerSketchSetColor::default();
        {
        ...
        let command = ClientCommandSketchSetColor::with_color(
            vec!["sk-1".to_string(), "sk-2".to_string()],
            0xFF00FF,
        );
        handler.update_sketch_manager(1, &command);
        // Sketch has no public rgb getter; ensure both sketches are still present and untouched otherwise.
        let mut mgr = handler.sketch_manager.lock().unwrap();
        assert_eq!(mgr.get_sketches("1").len(), 2);
    }

    #[test]
    fn create_server_command_carries_coach_ids_and_rgb() {
        let handler = ServerCommandHandlerSketchSetColor::default();
        let command =
            ClientCommandSketchSetColor::with_color(vec!["sk-1".to_string()], 0x00FF00);
        ...
        assert_eq!(server_command.get_rbg(), 0x00FF00);
    }
