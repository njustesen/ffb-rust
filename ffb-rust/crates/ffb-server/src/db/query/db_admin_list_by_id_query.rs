/// 1:1 translation of com.fumbbl.ffb.server.db.query.DbAdminListByIdQuery.
...
/// Manual MySQL DATETIME/TIMESTAMP -> epoch-millis conversion. mysql_common is not
/// compiled with a chrono/time feature in this workspace, so `Value::Date` is decoded
/// by hand (Howard Hinnant's days_from_civil algorithm).
...
pub struct DbAdminListByIdQuery;

impl DbAdminListByIdQuery {
    pub fn new() -> Self {
        Self
    }

    pub fn get_id(&self) -> DbStatementId {
        DbStatementId::ADMIN_LIST_BY_ID_QUERY
    }

    /// prepare() is a JDBC artifact — mysql_async does not need it.
    // pub fn prepare(&mut self, conn: &mut Conn) { /* no-op */ }

    pub async fn execute(&self, conn: &mut Conn, game_id: i64) -> Result<Vec<AdminListEntry>, DbError> {
        conn.exec_map(SQL, (game_id,), |mut row: Row| {
            let mut col = 0usize;
...
impl Default for DbAdminListByIdQuery {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn value_to_millis_handles_epoch() {
        assert_eq!(value_to_millis(Value::Date(1970, 1, 1, 0, 0, 0, 0)), Some(0));
    }
}
