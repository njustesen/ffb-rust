/// BB2020 end-of-game step sequence.
/// Mirrors Java `com.fumbbl.ffb.server.step.generator.bb2020.EndGame`.
...
impl EndGame {
    ...
    pub fn build_sequence(params: &EndGameParams) -> Vec<SequenceStep> {
        ...
        // 2 ASSIGN_TOUCHDOWNS (BB2020-specific)
        seq.add(StepId::AssignTouchdowns, vec![]);
        // 3 PENALTY_SHOOTOUT
...
mod tests {
    ...
    #[test]
    fn end_game_has_assign_touchdowns() {
        let steps = EndGame::build_sequence(&EndGameParams::default());
        assert!(steps.iter().any(|s| s.step_id == StepId::AssignTouchdowns));
    }

    #[test]
    fn end_game_has_penalty_shootout() {
        let steps = EndGame::build_sequence(&EndGameParams::default());
