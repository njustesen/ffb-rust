...
use ffb_mechanics::bb2020::throw_in_mechanic::ThrowInMechanic;
...
///   - Catch ball (regular, hand-off, accurate pass, kickoff, scatter, throw-in)
...
///   - No CATCH_PUNT case in regular catch fallthrough
///   - CATCH_KICKOFF goes directly to divingCatch (not a separate path for empty square)
///   - No Blast-it (grantsCatchBonusToReceiver) dialog in catchBall
///   - handle_command uses multi-select player list for diving catch
    ...
    fn handle_command(&mut self, action: &Action, game: &mut Game, rng: &mut GameRng) -> StepOutcome {
        match action {
            Action::SelectPlayer { player_id } => {
                // Java BB2020: CLIENT_PLAYER_CHOICE delivers List<String> selected.
                // In single-player-selection form (one ID or empty string):
                match self.phase {
                    ...
                    DivingCatchPhase::AskAway => {
                        if !player_id.is_empty() {
                            ...
                            if self.diving_catch_control_team.is_none() {
                                // Java: divingCatchControlTeam == null → getTeamAway, else getActingTeam
                                self.diving_catch_control_team = Some(game.team_away.id.clone());
                            }
                            // Java: divingCatchControlTeam != null → keep acting team id (already set)
                        }
            ...
            Action::UseSkill { skill_id: _, use_skill } => {
                // Java BB2020: only handles rerollSource != null path here.
                // No Blast-it (grantsCatchBonusToReceiver) path in BB2020.
                // If a skill provides a reroll source, set it (used=true) or clear it (used=false).
                if *use_skill {
                    // use_skill accepted — evaluate stays false in BB2020
                } else {
...
impl StepCatchScatterThrowIn {
    fn execute_step(&mut self, game: &mut Game, rng: &mut GameRng) -> StepOutcome {
        ...
        // Java BB2020 switch statement (with fallthrough):
        match self.catch_scatter_throw_in_mode {
            ...
            // Java BB2020: CATCH_ACCURATE_PASS, CATCH_HAND_OFF, CATCH_SCATTER — no CATCH_PUNT
            ...
            // Java BB2020: CATCH_KICKOFF, CATCH_THROW_IN, CATCH_ACCURATE_PASS_EMPTY_SQUARE, CATCH_MISSED_PASS
            // all share the same case (no CATCH_PUNT here)
            ...
            Some(CatchScatterThrowInMode::FailedDeflectionConversion) => {
                deflected_pass = true;
                // fall through to FAILED_CATCH handling
            ...
            // Java BB2020 has no explicit CATCH_PUNT case — treat as ScatterBall like default
    ...
    /// Java BB2020: handle CATCH_BOMB / CATCH_ACCURATE_BOMB / CATCH_ACCURATE_BOMB_EMPTY_SQUARE / DEFLECTED_BOMB
    ...
    /// Java BB2020: handle DEFLECTED / CATCH_ACCURATE_PASS / CATCH_HAND_OFF / CATCH_SCATTER
    /// (no CATCH_PUNT in BB2020)
    fn handle_regular_catch(&mut self, game: &mut Game, rng: &mut GameRng, deflected_pass: bool, player_under_ball: &Option<String>) {
        ...
        if let Some(ref cid) = self.catcher_id.clone() {
            let catcher_state = game.field_model.player_state(cid);
            if catcher_state.is_some() && game.field_model.ball_in_play && game.field_model.ball_moving {
                let has_tz = catcher_state.map(|s| s.has_tacklezones()).unwrap_or(false);
                if has_tz {
                    let new_mode = self.catch_ball(game, rng);
                    // Java BB2020: if null && deflectedPass && passState.isDeflectionSuccessful()
                    //   → passState.setInterceptionSuccessful(true)
                    // Java BB2020: if FAILED_CATCH && deflectedPass → FAILED_DEFLECTION_CONVERSION
    ...
    /// Java BB2020: handle FAILED_CATCH / FAILED_PICK_UP / FAILED_DEFLECTION_CONVERSION
    ///
    /// BB2020 includes spiked-ball check (SPIKED_BALL option or droppedBallCausesArmourRoll skill).
    /// The sub-sequence push is TODO pending full injury pipeline integration.
    ...
    /// Java BB2020: throwInBall() — uses BB2020 ThrowInMechanic (d6 direction, 2d6+1 distance).
    fn throw_in_ball(&mut self, game: &mut Game, rng: &mut GameRng) -> Option<CatchScatterThrowInMode> {
        ...
        // BB2020: isCornerThrowIn always returns false, so always d6 direction roll
    ...
    /// Java BB2020: catchBall() — AG roll, modifier lookup, success/fail/reroll paths.
    ///
    /// BB2020 differences vs BB2025:
    ///   - No Blast-it (grantsCatchBonusToReceiver) dialog
    ///   - No successfulWithBlastIt path
    ///   - No CATCH_PUNT in wrong-team check for END_TURN
    ///
    ...
    fn catch_ball(&mut self, game: &mut Game, rng: &mut GameRng) -> Option<CatchScatterThrowInMode> {
        ...
        if do_roll || self.evaluate {
            ...
            if successful {
                ...
                self.re_roll_state.re_rolled_action = None;
                // Java BB2020: if hand-off/accurate-pass caught by wrong team → END_TURN
                // (no CATCH_PUNT in BB2020)
        ...
        if let Some(coord) = catcher_coord {
            if self.phase != DivingCatchPhase::Process {
                if mode.is_bomb() {
                    ...
                    game.field_model.ball_moving = true;
                    // Java BB2020: passState.setUsingBlastIt(false) — skipped (no passState in model)
                }
            }
        }
        self.roll = 0;
...
mod tests {
    ...
    #[test]
    fn scatter_ball_in_play_advances_mode() {
        let mut game = make_game();
        ...
        // Mode should have advanced (no longer ScatterBall — bounce_ball ran once)
    ...
    // ── diving catch phase progression ──────────────────────────────────────────
    ...
    // ── BB2020 specific: no CATCH_PUNT in catch_kickoff group ────────────────────

    #[test]
    fn catch_kickoff_with_player_under_ball_becomes_catch_scatter() {
        let mut game = make_game();
        let coord = FieldCoordinate::new(12, 7);
        game.field_model.ball_coordinate = Some(coord);
        game.field_model.ball_in_play = true;
        game.field_model.ball_moving = true;
        // Put a player under the ball
        let player = make_player("h1", 4);
        game.team_home.players.push(player);
        game.field_model.set_player_coordinate("h1", coord);
        game.field_model.set_player_state("h1", PlayerState::new(PS_STANDING));
        ...
        let mut step = StepCatchScatterThrowIn::new();
        step.catch_scatter_throw_in_mode = Some(CatchScatterThrowInMode::CatchKickoff);
        // After execute_step, player is under ball → mode becomes CatchScatter
        step.start(&mut game, &mut GameRng::new(0));
        // Mode became CatchScatter (the player-under-ball path)
        assert_eq!(step.catch_scatter_throw_in_mode, Some(CatchScatterThrowInMode::CatchScatter));
    }
}
