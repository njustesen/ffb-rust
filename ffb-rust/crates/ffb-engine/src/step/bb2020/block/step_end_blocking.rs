...
use crate::step::generator::bb2020::Block;
use crate::step::generator::bb2020::block::BlockParams;
use crate::step::generator::bb2020::BlitzBlock;
...
impl StepEndBlocking {
    fn execute_step(&mut self, game: &mut Game, _rng: &mut GameRng) -> StepOutcome {
        ...
        let regular_block = !self.using_stab && !self.using_chainsaw
            && !self.using_vomit && !self.using_breathe_fire;
        ...
        if self.end_turn || self.end_player_action {
            ...
            // Java: endGenerator.pushSequence(EndPlayerAction.SequenceParams(gs, true, true, fEndTurn))
            // BB2020: no check_forgo field
        ...
        if std::env::var_os("FFB_TRACE").is_some() {
            eprintln!("REB is_blitz={is_blitz} can_move_on={can_move_on} tz={} next_move={next_move_possible} base={attacker_state_base} pa={player_action:?}",
                attacker_state.has_tacklezones());
        }
...
mod tests {
    ...
    fn add_player_to_home(game: &mut Game, pid: &str) {
        ...
        game.team_home.players.push(Player {
            ...
            temporary_properties: vec![], used_skills: HashSet::new(),
    ...
    fn pile_driver_true_pushes_pile_driver_sequence_and_returns_early() {
        ...
        let mut atk = Player { id: atk_id.clone(), name: "Attacker".into(), nr: 1,
            ...
            position_agility: 3, position_passing: 4, position_armour: 8, position_cost: 0,
