...
mod tests {
    ...
    #[test]
    fn extra_gfi_once_skill_used_adds_skill_use_report() {
        use ffb_model::model::player::Player;
        ...
        step.coordinate_to = Some(to);
        step.start(&mut game, &mut GameRng::new(0));
        assert!(game.report_list.has_report(ReportId::SKILL_USE), "ReportSkillUse(RUSH_ADDITIONAL_SQUARE_ONCE) must be added");
    }
    ...
    fn extra_gfi_once_not_triggered_within_free_rushes() {
        ...
        let to = FieldCoordinate::new(6, 5);
        game.acting_player.player_id = Some("p1".into());
        // movement=6, possibleFreeRushes=2, threshold=8; current_move=7 -> after +1=8 which is NOT > 8
        ...
        step.coordinate_to = Some(to);
        step.start(&mut game, &mut GameRng::new(0));
        assert!(!game.report_list.has_report(ReportId::SKILL_USE), "no report when within free rushes");
    }
}
