...
/// 1:1 translation of com.fumbbl.ffb.server.step.bb2025.move.StepPickUp.
///
/// Agility roll to pick up the ball.  Skips if `ignore` or ball not at player's square.
/// On success → NEXT_STEP (+ END_PLAYER_ACTION if SecureTheBall).
...
///   1. Skill re-roll (e.g. Sure Hands — property canRerollPickup) — auto-used (unless secureTheBall)
...
impl Step for StepPickUp {
    ...
    fn handle_command(&mut self, action: &Action, game: &mut Game, rng: &mut GameRng) -> StepOutcome {
        match action {
            ...
            // UseReRoll { true } and any other command (e.g. PickUpChoice) → execute
...
impl StepPickUp {
    fn execute_step(&mut self, game: &mut Game, rng: &mut GameRng) -> StepOutcome {
        // Java: secureTheBall = actingPlayer.getPlayerAction() == PlayerAction.SECURE_THE_BALL
    ...
    fn pick_up(&mut self, game: &mut Game, rng: &mut GameRng, player_id: Option<String>) -> StepOutcome {
        ...
        // Failure path — attempt re-roll if first failure
        let already_rerolled = self.re_roll_state.re_rolled_action
            .as_ref().map(|a| a.name == "PICKUP").unwrap_or(false);
        ...
        if !already_rerolled {
            ...
            // Skill re-roll (Java: UtilCards.getRerollSource) — skip if secureTheBall
            if !self.secure_the_ball {
...
mod tests {
    ...
    fn add_player_at_ball_with_skill(game: &mut Game, id: &str, skill: SkillId) {
        let coord = FieldCoordinate::new(5, 5);
        game.team_home.players.push(Player {
            id: id.into(), name: id.into(), nr: 1, position_id: "lineman".into(),
            player_type: PlayerType::Regular, gender: PlayerGender::Male,
    ...
    fn failure_without_reroll_goes_to_label() {
        ...
        let mut step = StepPickUp::new("fail".into());
        step.roll = 1; // guaranteed fail (minimum >= 2)
        let out = step.start(&mut game, &mut GameRng::new(0));
    ...
    #[test]
    fn secure_the_ball_sets_flag_from_player_action() {
        let mut game = make_game();
        add_player_at_ball(&mut game, "p1");
        ...
        step.roll = 6;
        let out = step.start(&mut game, &mut GameRng::new(0));
        assert!(step.secure_the_ball, "secure_the_ball should be set from player action");
        assert_eq!(out.action, StepAction::NextStep);
    }

    #[test]
    fn secure_the_ball_success_publishes_end_player_action() {
        let mut game = make_game();
        add_player_at_ball(&mut game, "p1");
        game.acting_player.player_action = Some(PlayerAction::SecureTheBall);
        let mut step = StepPickUp::new("fail".into());
        step.roll = 6; // guaranteed success
        let out = step.start(&mut game, &mut GameRng::new(0));
        let published: Vec<_> = out.published.iter()
            .filter(|p| matches!(p, StepParameter::EndPlayerAction(_)))
            .collect();
        assert!(!published.is_empty(), "secureTheBall success should publish EndPlayerAction");
        if let StepParameter::EndPlayerAction(v) = published[0] {
            assert!(v, "EndPlayerAction should be true");
        }
    }

    #[test]
    fn secure_the_ball_not_set_without_secure_action() {
        let mut game = make_game();
        add_player_at_ball(&mut game, "p1");
        ...
        assert!(!step.secure_the_ball, "secure_the_ball should NOT be set for Move action");
    }
}
