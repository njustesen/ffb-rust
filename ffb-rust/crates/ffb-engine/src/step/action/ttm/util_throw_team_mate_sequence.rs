...
pub fn scatter_player(game: &Game, rng: &mut GameRng, start: FieldCoordinate, throw_scatter: bool) -> ScatterResult {
...
pub fn kick_player(game: &Game, rng: &mut GameRng, start: FieldCoordinate, target: FieldCoordinate) -> ScatterResult {
    let mut last_valid = start;
...
mod tests {
    ...
    fn scatter_player_bounce_stops_after_first_step_bb2025() {
        ...
        let _result = scatter_player(&game, &mut rng, start, false);
    ...
    fn kick_player_walks_toward_target() {
        ...
        let result = kick_player(&game, &mut rng, start, target);
