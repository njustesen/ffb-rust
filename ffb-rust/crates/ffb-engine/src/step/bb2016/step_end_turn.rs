...
/// - TurnMode skip guard (BLITZ/KICKOFF_RETURN/PASS_BLOCK/ILLEGAL_SUBSTITUTION) → publish END_TURN + NEXT_STEP
...
///
/// BB2016 differs from BB2020/BB2025:
/// - No Swarming TurnMode in the skip guard (BB2020/BB2025 addition)
/// - No useStarOfTheShow field
/// - No getPrayerHandlerFactory / getFaintingCount logic
///
/// Stubs (untranslated server-side systems):
/// - ArgueTheCall, Bribes dialogs → skip (choices set to Some(false) immediately)
...
mod tests {
    ...
    #[test]
    fn use_bribe_sets_within_secret_weapon_handling() {
        let mut game = make_game();
        let mut step = StepEndTurn::new();
