...
impl StepPenaltyShootout {
    ...
    fn execute_step(game: &mut Game, rng: &mut GameRng) -> StepOutcome {
            ...
            && game.game_result.home.score == game.game_result.away.score
        {
            ...
            // Java: getResult().addReport(new ReportPenaltyShootout(rollHome, reRollsLeftHome, rollAway, reRollsLeftAway))
            game.report_list.add(ReportPenaltyShootout::new(roll_home, rerolls_home, roll_away, rerolls_away));
        }
        StepOutcome::next()
    }
}
...
mod tests {
    ...
    #[test]
    fn overtime_tie_adds_penalty_shootout_report() {
        ...
        let mut rng = GameRng::new(42);
        step.start(&mut game, &mut rng);
        assert!(game.report_list.has_report(ReportId::PENALTY_SHOOTOUT));
    }

    #[test]
    fn no_tie_no_report_added() {
        use ffb_model::report::report_id::ReportId;
        let mut step = StepPenaltyShootout::new();
        let mut game = make_game();
        game.half = 3;
