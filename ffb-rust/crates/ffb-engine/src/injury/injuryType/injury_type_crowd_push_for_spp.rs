/// Translation of com.fumbbl.ffb.server.injury.injuryType.InjuryTypeCrowdPushForSpp.
...
pub struct InjuryTypeCrowdPushForSpp { ctx: InjuryContext }
impl InjuryTypeCrowdPushForSpp { pub fn new() -> Self { Self { ctx: InjuryContext::new(ApothecaryMode::Defender) } } }
...
mod tests {
    ...
    fn injury_is_reserve_or_ko_or_casualty() {
        for seed in 1..=10u64 {
            let mut t = InjuryTypeCrowdPushForSpp::new(); let mut rng = GameRng::new(seed);
            t.handle_injury(&make_game(), &mut rng, None, "p1", coord(), None, None, ApothecaryMode::Defender);
    ...
    fn context_stores_defender_id() {
        let mut t = InjuryTypeCrowdPushForSpp::new(); let mut rng = GameRng::new(1);
        t.handle_injury(&make_game(), &mut rng, None, "crowd_victim", coord(), None, None, ApothecaryMode::Defender);
        assert_eq!(t.ctx.defender_id.as_deref(), Some("crowd_victim"));
    }
    #[test]
    fn default_equivalent_to_new() {
        let t1 = InjuryTypeCrowdPushForSpp::new();
        let t2 = InjuryTypeCrowdPushForSpp::default();
