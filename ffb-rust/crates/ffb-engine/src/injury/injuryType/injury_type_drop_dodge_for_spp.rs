...
pub struct InjuryTypeDropDodgeForSpp {
    ctx: InjuryContext,
    arm_bar_player_id: Option<String>,
}

impl InjuryTypeDropDodgeForSpp {
    pub fn new() -> Self { Self { ctx: InjuryContext::new(ApothecaryMode::Defender), arm_bar_player_id: None } }
    pub fn new_with_arm_bar(arm_bar_player_id: Option<String>) -> Self {
        Self { ctx: InjuryContext::new(ApothecaryMode::Defender), arm_bar_player_id }
    }
}
impl Default for InjuryTypeDropDodgeForSpp { fn default() -> Self { Self::new() } }

impl InjuryTypeServer for InjuryTypeDropDodgeForSpp {
    fn java_type_name(&self) -> &'static str { "dropDodgeForSpp" }
    fn handle_injury(&mut self, game: &Game, rng: &mut GameRng, attacker_id: Option<&str>, defender_id: &str,
        ...
        if self.ctx.armor_broken {
            ...
            // isStab(), isFoul(), isVomitLike())` — DropDodgeForSpp doesn't override any of
...
mod tests {
    ...
    fn armor_save_results_in_prone() {
        let mut t = InjuryTypeDropDodgeForSpp::new(); let mut rng = GameRng::new(1);
    ...
    /// isStab/isFoul/isVomitLike are all false for DropDodgeForSpp, so Mighty Blow
    /// applies normally — proves the factory is now reached.
