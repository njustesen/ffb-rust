/// 1:1 translation of `com.fumbbl.ffb.server.mechanic.mixed.SetupMechanic`.
///
/// BB2016/BB2020 setup validation and pin-players-in-tacklezones.
/// Reads field limits from `game.options` (via `UtilGameOption`).
...
impl SetupMechanicTrait for SetupMechanic {
    ...
    /// Returns `true` if valid; returns `false` and logs messages if invalid.
    fn check_setup_with_swarmers(
        ...
        // Gather per-player data without borrowing game mutably mid-loop.
        let player_data: Vec<(String, bool, bool)> = {
            ...
            team.players.iter().map(|p| {
                ...
                let is_swarmer = p.has_skill_property(NamedProperties::CAN_SNEAK_EXTRA_PLAYERS_ONTO_PITCH);
                (p.id.clone(), is_keen, is_swarmer)
            }).collect()
        };

        for (player_id, is_keen, is_swarmer) in &player_data {
    ...
    ///
    /// Marks active players as inactive when adjacent to an opponent tacklezone,
    /// or (when `pin_ball_and_chain=true`) the player has the `movesRandomly` property.
...
mod tests {
    ...
    #[test]
    fn check_setup_empty_team_no_options_is_valid() {
        // No players, no options set → all counts are 0, limits are 0 → valid.
    ...
    #[test]
    fn pin_players_in_tacklezones_empty_team_is_noop() {
        let mut game = make_game();
        mechanic().pin_players_in_tacklezones(&mut game, "home");
    }

    #[test]
    fn pin_players_in_tacklezones_chain_both_variants_no_panic() {
        let mut game = make_game();
    ...
    #[test]
    fn check_setup_options_read_from_game() {
        let mut game = make_game();
    ...
    #[test]
    fn check_setup_too_many_players_is_invalid() {
        let mut game = make_game();
        game.options.set(game_option_id::MAX_PLAYERS_ON_FIELD, "2");
        game.options.set(game_option_id::MAX_PLAYERS_IN_WIDE_ZONE, "2");
        game.options.set(game_option_id::MIN_PLAYERS_ON_LOS, "0");
        // Home half: x=1..8, so place 3 players in home half
        add_player(&mut game, "home", "p1", FieldCoordinate::new(5, 3));
        add_player(&mut game, "home", "p2", FieldCoordinate::new(5, 4));
        add_player(&mut game, "home", "p3", FieldCoordinate::new(5, 5));
        assert!(!mechanic().check_setup(&mut game, true));
    }
    ...
    fn pin_players_pins_player_adjacent_to_opponent_with_tacklezone() {
        let mut game = make_game();
        // Place home player at (5,5) and away player at (6,5) — adjacent.
        // Both active, away player has tacklezone (default active state).
        add_player(&mut game, "home", "h1", FieldCoordinate::new(5, 5));
        add_player(&mut game, "away", "a1", FieldCoordinate::new(6, 5));

        mechanic().pin_players_in_tacklezones_chain(&mut game, "home", false);

        // Home player should be pinned (inactive).
    ...
    fn pin_players_does_not_pin_player_not_adjacent_to_opponent() {
        let mut game = make_game();
        // Place home player far from away player.
    ...
    #[test]
    fn pin_ball_and_chain_pins_moves_randomly_player() {
        let mut game = make_game();
        // Home player with MOVES_RANDOMLY (BallAndChain) at (5,5), no adjacent opponents.
        add_player_with_skill(&mut game, "home", "h1", FieldCoordinate::new(5, 5), SkillId::BallAndChain);

        mechanic().pin_players_in_tacklezones_chain(&mut game, "home", true);

        let state = game.field_model.player_state("h1").unwrap();
        assert!(!state.is_active(), "BallAndChain player should be pinned when pin_ball_and_chain=true");
    }
    ...
    fn pin_ball_and_chain_false_does_not_pin_moves_randomly() {
        ...
        let state = game.field_model.player_state("h1").unwrap();
        assert!(state.is_active(), "BallAndChain player should not be pinned when pin_ball_and_chain=false and no adjacent opponents");
    }
}
