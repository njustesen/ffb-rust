/// 1:1 translation of BB2016 `ChopBlockHandler`.
...
impl CardHandler for ChopBlockHandler {
    ...
    /// Java: playerState.isActive() && !isProneOrStunned() && adjacentBlockableOpponents.length > 0
    /// Java uses inducement set to find ownTeam; we fall back to team membership (ownTeam TBD).
...
mod tests {
    ...
    // Java's allowsPlayer takes a Player object, a missing id is inexpressible.
