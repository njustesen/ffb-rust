/// 1:1 translation of BB2016 `CustardPieHandler`.
...
impl CardHandler for CustardPieHandler {
    ...
    /// Java: findAdjacentStandingOrPronePlayers(game, ownTeam, playerCoordinate).length > 0
    /// BB2016 differs from BB2020: allows standing OR prone neighbors (not just blockable/active).
    fn allows_player(&self, game: &Game, _card: &Card, player_id: &str) -> bool {
        ...
        // Java: findAdjacentStandingOrPronePlayers = adjacent own-team players that are not stunned
        let adjacent = UtilPlayer::find_adjacent_players(game, own_team, coord);
        adjacent.iter().any(|id| {
            game.field_model.player_state(id).map(|s| !s.is_stunned()).unwrap_or(false)
        })
    }
