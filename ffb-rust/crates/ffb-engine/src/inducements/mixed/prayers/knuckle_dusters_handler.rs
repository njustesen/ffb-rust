...
/// Extends SelectPlayerPrayerHandler — coach selects a player who gains Mighty Blow (+1).
...
pub fn remove_effect_internal(_prayer_state: &mut PrayerState, game: &mut Game, team_id: &str) {
    EnhancementRemover::new().remove_enhancement(game, team_id, "KNUCKLE_DUSTERS");
}
...
mod tests {
    ...
    fn remove_effect_clears_enhancement() {
        ...
        game.field_model.add_prayer_enhancement("p1", "KNUCKLE_DUSTERS");
        assert!(game.field_model.has_prayer_enhancement("p1", "KNUCKLE_DUSTERS"));
