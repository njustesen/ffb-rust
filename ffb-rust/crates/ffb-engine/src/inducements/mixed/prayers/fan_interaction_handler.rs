/// 1:1 translation of `com.fumbbl.ffb.server.inducements.mixed.prayers.FanInteractionHandler`.
use ffb_model::model::animation_type::AnimationType;
use crate::prayer_state::PrayerState;

pub fn init_effect(prayer_state: &mut PrayerState, team_id: &str) -> bool {
    prayer_state.add_fan_interaction(team_id);
    true
}

pub fn remove_effect_internal(prayer_state: &mut PrayerState, team_id: &str) {
    prayer_state.remove_fan_interaction(team_id);
}

pub fn animation_type() -> AnimationType {
    AnimationType::PRAYER_FAN_INTERACTION
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn init_effect_adds_fan_interaction() {
        let mut state = PrayerState::new();
        assert!(init_effect(&mut state, "team1"));
        assert!(state.has_fan_interaction("team1"));
    }

    #[test]
    fn remove_effect_removes_fan_interaction() {
        let mut state = PrayerState::new();
        state.add_fan_interaction("team1");
        remove_effect_internal(&mut state, "team1");
        assert!(!state.has_fan_interaction("team1"));
    }

    #[test]
    fn animation_type_is_correct() {
        assert_eq!(animation_type(), AnimationType::PRAYER_FAN_INTERACTION);
    }

    #[test]
    fn init_effect_does_not_affect_other_team() {
        let mut state = PrayerState::new();
        init_effect(&mut state, "team1");
        assert!(!state.has_fan_interaction("team2"));
    }

    #[test]
    fn remove_effect_on_missing_team_is_safe() {
        let mut state = PrayerState::new();
        remove_effect_internal(&mut state, "team_not_present");
        assert!(!state.has_fan_interaction("team_not_present"));
    }
}
