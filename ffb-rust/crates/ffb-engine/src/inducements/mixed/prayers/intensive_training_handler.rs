...
/// Extends SelectPlayerPrayerHandler — coach selects a random player who gains a chosen Primary skill.
...
pub fn apply_selection(_prayer_state: &mut PrayerState, _game: &mut Game, _player_id: &str) {
    // DEFERRED(prayer-dialog): needs fieldModel.addPrayerEnhancements for INTENSIVE_TRAINING
}
...
mod tests {
    ...
    fn remove_effect_clears_enhancement() {
        ...
        game.field_model.add_prayer_enhancement("p1", "INTENSIVE_TRAINING");
        assert!(game.field_model.has_prayer_enhancement("p1", "INTENSIVE_TRAINING"));
