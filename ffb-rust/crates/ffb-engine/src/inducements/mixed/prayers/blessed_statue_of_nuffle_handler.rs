...
/// Extends SelectPlayerPrayerHandler — selects 1 random player who gains Pro.
...
pub const PRAYER_NAME: &str = "BLESSED_STATUE_OF_NUFFLE";

pub fn animation_type() -> AnimationType {
    AnimationType::PRAYER_BLESSED_STATUE_OF_NUFFLE
}

/// Java: initEffect — selects 1 RESERVE player on the praying team, marks prayer, grants Pro.
pub fn init_effect(prayer_state: &mut PrayerState, game: &mut Game, rng: &mut GameRng, team_id: &str, selector: &dyn PlayerSelector) -> bool {
    // Java BlessedStatueOfNuffleHandler is a SelectPlayerPrayerHandler (dialog) whose
    // eligibility uses skillsFromEnhancement = {Pro}; this random-selection port keeps that set.
    init_effect_random_selection(prayer_state, game, rng, team_id, PRAYER_NAME, 1, selector, &[SkillId::Pro])
}
...
mod tests {
    ...
    #[test]
    fn apply_selection_marks_and_grants_pro() {
        let mut state = PrayerState::new();
        let mut game = make_game();
        add_reserve_player(&mut game, "h1");
