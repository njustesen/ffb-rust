/// 1:1 translation of com.fumbbl.ffb.server.skillbehaviour.bb2016.GrabBehaviour.
///
/// Priority 3 modifier on StepPushback.
///
/// **BB2016 vs BB2020/BB2025 differences:** no ViciousVines adjacency alternative, no
/// attacker-conflicting-skill check, and uses an exact `PlayerAction::Block` match instead of
/// the generic `is_block_action()`/`isBlockAction()` helper the later editions added.
...
impl StepModifierTrait for GrabStepModifier {
    ...
    fn handle_execute_step(
        ...
        let is_adjacent = attacker_coord
            ...
            .unwrap_or(false);
        // Java: actingPlayer.getPlayerAction() == PlayerAction.BLOCK (exact match, not isBlockAction())
        let is_block = game.acting_player.player_action
            .map(|a| a == ffb_model::enums::PlayerAction::Block)
            .unwrap_or(false);
        ...
        if (state.grabbing.is_none() || state.grabbing == Some(true))
            ...
            && is_adjacent
            && !has_cancelling
            && (is_block || is_multiple_block || allow_grab_outside_block)
        {
...
mod tests {
    ...
    /// BB2016-specific: PlayerAction::MultipleBlock alone (without exact Block) still qualifies.
    #[test]
    fn grab_fires_on_multiple_block_action() {
        let mut game = make_game();
        game.team_home.players.push(player_with_skills("att", vec![SkillId::Grab]));
        game.acting_player.player_id = Some("att".into());
