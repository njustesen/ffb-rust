...
/// Lone Fouler: Dirty Player bonus applies even if no teammate assists the foul.
/// No StepModifier -- execute_step_hook is a no-op (returns the default false).
pub struct LoneFoulerBehaviour;

impl LoneFoulerBehaviour {
    pub fn new() -> Self { Self }
}

impl Default for LoneFoulerBehaviour {
    fn default() -> Self { Self::new() }
}

impl SkillBehaviour for LoneFoulerBehaviour {
    fn name(&self) -> &'static str { "LoneFoulerBehaviour" }
}
...
mod tests {
    ...
    fn hook_is_noop_returns_false() {
        let behaviour = LoneFoulerBehaviour::new();
...
#[test]    fn name_is_not_empty() {        assert!(!LoneFoulerBehaviour::new().name().is_empty());    }    #[test]    fn execute_step_hook_false_with_bb2025() {        use ffb_model::enums::Rules;        use crate::step::framework::test_team;        let b = LoneFoulerBehaviour::new();        let mut game = ffb_model::model::game::Game::new(            test_team("home", 0), test_team("away", 0), Rules::Bb2025,        );        assert!(!b.execute_step_hook(&mut game));    }
}
