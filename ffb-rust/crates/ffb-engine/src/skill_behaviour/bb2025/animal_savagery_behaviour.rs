...
impl SkillBehaviour for AnimalSavageryBehaviour {
    ...
    fn execute_step_hook(&self, game: &mut ffb_model::model::game::Game) -> bool {
        // Java StepModifier.handleExecuteStepHook: checks state.playerId, negatrait eligibility, UtilCards.hasSkill(actingPlayer, skill), state.attackOpponent, rolls dice.
        ...
        // TODO(hook-infra): step-specific state access (StepState.playerId, StepState.attackOpponent, game.getTurnMode().checkNegatraits()) not yet available
        false
    }
}
...
mod tests {
    ...
    fn hook_is_noop_returns_false() {
        let behaviour = AnimalSavageryBehaviour::new();
