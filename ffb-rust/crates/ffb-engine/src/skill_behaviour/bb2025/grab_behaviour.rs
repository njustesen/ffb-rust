...
impl StepModifierTrait for GrabStepModifier {
    ...
    ///
    /// Conditions for Grab to fire:
    ///   - (grabbing == null || grabbing == true)
    ///   - freeSquareAroundDefender
    ///   - actingPlayer has Grab
    ///   - attackerCoordinate is adjacent to defenderCoordinate (or ViciousVines)
    ///   - no cancelling skill on defender
    ///   - attacker does not have a conflicting skill (property: canPushBackToAnySquare conflicts with itself)
    ///   - action is a block action (or MultipleBlock, or grabOutsideBlock property)
    ///
    /// Returns true if Grab was handled (either used or decided); false if conditions not met.
    fn handle_execute_step(
        ...
        // Java: FieldCoordinate attackerCoordinate = fieldModel.getPlayerCoordinate(actingPlayer.getPlayer())
        let attacker_coord = game.field_model.player_coordinate(&attacker_id);
        // Java: FieldCoordinate defenderCoordinate = state.startingPushbackSquare.getCoordinate()
        ...
        // Grab conflicts with itself — if attacker somehow has conflicting skill
        ...
        // Java: attackerCoordinate.isAdjacent(defenderCoordinate)
            ...
            && (is_block_action || is_multiple_block || allow_grab_outside_block)
        {
            ...
            //   state.grabbing = true;
            //   for each pushbackSquare: if fieldModel.getPlayer(sq.coordinate) != null → state.grabbing = null; break
            ...
            // Java: if (state.grabbing) { GRAB mode } else { state.grabbing = false }
            if state.grabbing == Some(true) {
                state.pushback_mode = PushbackMode::GRAB;
                // Java: fieldModel.remove non-selected squares; rebuild in GRAB mode
                ...
                // Java: state.grabbing = false (explicitly declined)
...
mod tests {
    ...
    fn grab_not_adjacent_returns_false() {
        ...
        game.acting_player.player_id = Some("att".into());
        game.acting_player.player_action = Some(PlayerAction::Block);
        // Place attacker far from defender
    ...
    fn grab_with_free_pushback_squares_auto_grabs() {
        ...
        let m = GrabStepModifier;
        let mut hs = default_hook_state_with_sq("def", sq);
        // Add some free pushback squares
