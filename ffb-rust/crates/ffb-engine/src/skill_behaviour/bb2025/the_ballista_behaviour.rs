...
/// The Ballista: when throwing a teammate, passing range is extended by the thrower's Strength.
...
impl SkillBehaviour for TheBallistaBehaviour {
    ...
    fn execute_step_hook(&self, game: &mut ffb_model::model::game::Game) -> bool {
        // Java StepModifier.handleExecuteStepHook: checks acting player has TheBallista.
        let has_skill = game.acting_player.player_id.as_deref()
            .and_then(|id| game.player(id))
            .map(|p| p.has_skill(SkillId::TheBallista))
            .unwrap_or(false);
        if !has_skill {
            return false;
        }
        // TODO(hook-infra): step-specific state access (StepState throw-teammate range extension by Strength) not yet available
        false
    }
}
...
mod tests {
    ...
    fn hook_is_noop_returns_false() {
        let behaviour = TheBallistaBehaviour::new();
...
#[test]    fn name_is_not_empty() {        assert!(!TheBallistaBehaviour::new().name().is_empty());    }    #[test]    fn execute_step_hook_false_with_bb2025() {        use ffb_model::enums::Rules;        use crate::step::framework::test_team;        let b = TheBallistaBehaviour::new();        let mut game = ffb_model::model::game::Game::new(            test_team("home", 0), test_team("away", 0), Rules::Bb2025,        );        assert!(!b.execute_step_hook(&mut game));    }
}
