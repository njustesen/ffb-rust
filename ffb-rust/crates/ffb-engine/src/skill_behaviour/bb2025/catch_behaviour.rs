...
/// Mirrors Java `com.fumbbl.ffb.server.skillbehaviour.bb2025.CatchBehaviour`.
...
impl SkillBehaviour for CatchBehaviour {
    ...
    fn execute_step_hook(&self, game: &mut ffb_model::model::game::Game) -> bool {
        // Java StepModifier.handleExecuteStepHook: checks UtilCards.hasSkill(state.catcher, skill)
        // -- note: uses state.catcher (the player catching the ball), not the acting player.
        // Returns true if the catcher has the Catch skill (step is consumed).
        ...
        // TODO(hook-infra): step-specific state access (StepState.catcher, re-roll dialog)
        //   not yet available -- should return true when catcher has Catch skill
        false
    }
}
...
mod tests {
    ...
    fn hook_is_noop_returns_false() {
        let behaviour = CatchBehaviour::new();
...
#[test]    fn name_is_not_empty() {        assert!(!CatchBehaviour::new().name().is_empty());    }    #[test]    fn execute_step_hook_false_with_bb2025() {        use ffb_model::enums::Rules;        use crate::step::framework::test_team;        let b = CatchBehaviour::new();        let mut game = ffb_model::model::game::Game::new(            test_team("home", 0), test_team("away", 0), Rules::Bb2025,        );        assert!(!b.execute_step_hook(&mut game));    }
}
