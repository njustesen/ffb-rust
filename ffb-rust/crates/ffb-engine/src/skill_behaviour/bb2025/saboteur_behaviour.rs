...
/// Saboteur: player does not need a free block die to foul; auto-escapes ejection on 1.
...
impl SkillBehaviour for SaboteurBehaviour {
    ...
    fn execute_step_hook(&self, game: &mut ffb_model::model::game::Game) -> bool {
        // Java StepModifier.handleExecuteStepHook: checks acting player has Saboteur.
        ...
        // TODO(hook-infra): step-specific state access (StepState foul ejection logic, ejection bypass) not yet available
        false
    }
}
...
mod tests {
    ...
    fn hook_is_noop_returns_false() {
        let behaviour = SaboteurBehaviour::new();
...
#[test]    fn name_is_not_empty() {        assert!(!SaboteurBehaviour::new().name().is_empty());    }    #[test]    fn execute_step_hook_false_with_bb2025() {        use ffb_model::enums::Rules;        use crate::step::framework::test_team;        let b = SaboteurBehaviour::new();        let mut game = ffb_model::model::game::Game::new(            test_team("home", 0), test_team("away", 0), Rules::Bb2025,        );        assert!(!b.execute_step_hook(&mut game));    }
}
