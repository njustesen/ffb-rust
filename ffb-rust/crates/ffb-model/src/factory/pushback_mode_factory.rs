...
mod tests {
    ...
    fn initialize_does_not_panic() {
        let mut f = PushbackModeFactory::default();
        f.initialize();
    }

    #[test]
    fn for_name_a_second_known_variant() {
        assert_eq!(PushbackModeFactory::default().for_name("grab"), Some(PushbackMode::GRAB));
    }

    #[test]
    fn for_name_empty_string_returns_none() {
        assert_eq!(PushbackModeFactory::default().for_name(""), None);
    }
}
