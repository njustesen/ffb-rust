...
mod tests {
    ...
    fn initialize_does_not_panic() {
        let mut f = PassingDistanceFactory::default();
        f.initialize();
    }

    #[test]
    fn for_name_a_second_known_variant() {
        assert_eq!(PassingDistanceFactory::default().for_name("Long Bomb"), Some(PassingDistance::LongBomb));
    }

    #[test]
    fn for_name_empty_string_returns_none() {
        assert_eq!(PassingDistanceFactory::default().for_name(""), None);
    }
}
