...
mod tests {
    ...
    fn initialize_does_not_panic() {
        let mut f = ConcedeGameStatusFactory::default();
        f.initialize();
    }

    #[test]
    fn for_name_a_second_known_variant() {
        assert_eq!(ConcedeGameStatusFactory::default().for_name("denied"), Some(ConcedeGameStatus::DENIED));
    }

    #[test]
    fn for_name_empty_string_returns_none() {
        assert_eq!(ConcedeGameStatusFactory::default().for_name(""), None);
    }
}
