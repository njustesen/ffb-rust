...
mod tests {
    ...
    fn initialize_does_not_panic() {
        let mut f = PassResultFactory::default();
        f.initialize();
    }

    #[test]
    fn for_name_a_second_known_variant() {
        assert_eq!(PassResultFactory::default().for_name("inaccurate"), Some(PassResult::Inaccurate));
    }

    #[test]
    fn for_name_empty_string_returns_none() {
        assert_eq!(PassResultFactory::default().for_name(""), None);
    }
}
