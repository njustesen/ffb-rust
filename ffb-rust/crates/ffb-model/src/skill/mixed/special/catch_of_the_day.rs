...
mod tests {
    ...
    #[test]
    fn name_is_catch_of_the_day() {
        assert_eq!(CatchOfTheDay::new().get_name(), "Catch of the Day");
    }

    #[test]
    fn category_is_trait() {
        assert_eq!(CatchOfTheDay::new().get_category(), SkillCategory::Trait);
    }
    ...
    fn has_skill_properties_not_null() {
        ...
        let props: &'static [&'static str] = crate::enums::SkillId::CatchOfTheDay.properties();
        assert!(props.iter().all(|p| !p.is_empty()));
    }

    #[test]
    fn usage_type_is_once_per_half() {
        assert_eq!(CatchOfTheDay::new().get_skill_usage_type(), SkillUsageType::OncePerHalf);
    }
}
