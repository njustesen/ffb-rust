...
**§12 iter 70b - CORRECTION to the correction: nothing is carried forward. The divergence is
INSIDE away_03's blitz.**

Compared the full `state=` string at the first five activations on both builds:

    i=1  identical      i=2  identical      i=3  identical
    i=4  identical  <-- away_03's Blitz DECLARATION, byte-identical state on both
    i=5  rng=26 (main) vs rng=27 (branch)

So the "state carried forward from home_01's blitz" reading in the previous entry is wrong too -
the engines enter away_03's activation in exactly the same state and leave it one die apart. The
extra `CatchScatterThrowIn` is generated by the chain's own resolution of THAT blitz.

The roll sits after `PickUp` in the BlitzBlock sequence, and PickUp fires when a player ends on
the ball's square. So the likeliest cause is that the blitzer ENDS SOMEWHERE DIFFERENT on the
branch - which would not itself cost a die. Note the stand-up-cost fix (iteration 54) deliberately
changed `current_move` on the chain path, and movement allowance is exactly what decides where a
blitzer stops.
