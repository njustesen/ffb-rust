...
## Iter 37 — seed 30 i=154 FIXED (no-defender plain block deselects); seeds 1-40 green

Root cause (seed 30 i=154, Activate(away_02, BLOCK)): the turn-start eligibility snapshot offered away_02
(a01 at (13,6)) a BLOCK, but by activation its only adjacent opponent had moved/been knocked away — no
blockable target remained (nearest home players were 2 squares off). Both agents still pick BLOCK from the
cached snapshot (Iter-27 pattern), then need a defender. Java's ParityRunner.sendBlockAction: pickBlockTarget
returns null → it DESELECTS (ClientCommandActingPlayer(null) → END_PLAYER_ACTION), and since away_02 was the
last unused away player the turn then ends (i=155 = home). Rust's agent supplies block_defender_id=None and
StepInitBlocking's no-defender path returned cont() (wait for a CLIENT_BLOCK that never comes) for a plain
block — the headless runner then broke on the promptless wait (Rust log truncated 156 vs Java 278).

The Iter-27 fix had handled the no-defender BLITZ (→ EndTurn, BLITZ_TARGET_NONE) but deliberately left plain
BLOCK on cont() to preserve the interactive GUI wait-for-command path. Fix (step_init_blocking.rs): on a
no-defender plain block, distinguish the two cases via legal_block_targets — if NO adjacent blockable target
exists, DESELECT (publish EndPlayerAction) to mirror Java's sendBlockAction; if a target DOES exist (defender
just not chosen yet), keep waiting (cont()) for the GUI path. Tests: no_defender_plain_block_deselects_ends_
...
## Iter 44 — seed 22 FIXED: deselect a no-receiver HandOver instead of stalling
- **Symptom:** seed 22 the Rust game ended ~150 steps early (i=184 `Activate(away_04,HandOff)`, turn 2 half 2). away_04 = ball carrier at (13,8); the HandOff produced `prompt_after=None finished=false` — a silent stall. `FFB_DRIVE_TRACE` ended at `InitPassing stack_len=28`.
- **Root cause:** the parity agent snapshots each player's eligible actions at TURN START (mirrors Java `eligibleThisTurn`) and only filters single-use flags. away_04 had an adjacent teammate at turn start → HandOver captured; the teammate then moved away. Both engines still offer HandOff (aligned), both pick it. Java's `ParityRunner.sendHandOverAction` finds **no adjacent receiver → deselects** (`ClientCommandActingPlayer(null,null,false)`), turn continues. Rust's `legal_handoff_receivers` returns empty → `block_defender_id=None` → `game.defender_id` unset → no `TargetCoordinate` → `StepInitPassing` runs with no thrower → `return cont()` with no prompt → drive stalls (drive loop parks on `waiting_for_command` with nothing to feed it).
- **Fix (`step_init_selecting.rs`):** (1) `handle_command` now clears `game.defender_id = None` when `block_defender_id` is None (kills stale-defender carryover); (2) the dispatch block, for a `HandOver | Pass` dispatch with `game.defender_id.is_none()`, returns `goto(end_label)` + `EndPlayerAction(true)` — the faithful analog of Java's Phase-2 empty-target deselect. No extra RNG (the action-pick already happened; the empty receiver list consumes zero pick-RNG in both engines).
- **Test:** `hand_over_activation_without_receiver_deselects` (also asserts a stale defender_id can't resurrect the hand-over).
- **Result:** PARITY 25/25 (seeds 1-25 green). Frontier ≥ 26. ffb-engine 7005/0.

## Iter 45 (IN PROGRESS) — seed 46 frontier: dodge reroll divergence (UNSOLVED)
- **Status:** seeds 1-45 GREEN (Iter43+44). Seed 46 is the frontier. NOT yet fixed — investigation notes below.
- **Divergence:** seed 46 i=176 (turn 1 half 2, home). home_02 (h01) at (12,6), adjacent to away_02(13,6)+away_01(13,7), dodges one square to (11,7) and FAILS → falls. Java rolls the dodge die TWICE (DICE_TRACE pos47=4, pos48=1, both `StepMoveDodge.dodge:309` via `rollSkill:101`) then armour(2,5) + injury(3,1) → **Stunned**. Rust rolls the dodge ONCE (pos47=4), fails, offers the TRR, agent NoReRoll → armour holds → **Prone**. So Java gets a SECOND dodge die Rust doesn't → 1-die desync → downstream away_01 blitz on ball-carrier home_03 rolls different block dice → Rust turnover vs Java continue.
- **Ruled out:** it is NOT the team-reroll decision. Java ParityRunner RE_ROLL handler calls `sendUseReRoll(action, null)` — the null reRollSource means DECLINE (confirmed: forcing Rust `use_reroll:true` FIXED seed46's dodge but REGRESSED seed 1, where Java declines). Reverted. Parity linemen have NO starting skills (make_lineman_team: AV8 AG3 MA6, starting_skills=[]).
- **Leading hypothesis:** Java's extra dodge die is an AUTO-APPLIED reroll from a reroll SOURCE that is a skill/prayer/kickoff grant (used by the engine without a RE_ROLL dialog), which Rust isn't applying — OR a dodge that Rust resolves as one roll where Java's StepMoveDodge re-enters `dodge()` (line 240 `setReRolledAction(DODGE)`). NEXT: determine what reRollSource Java uses for home_02's dodge in seed 46 (grep Java trace for a ReportReRoll/prayer/kickoff-skill grant near i=176; check the half-2 kickoff event; check whether home_02 gained Dodge/a reroll via a Prayer To Nuffle). The jar's `StepMoveDodge.dodge:309` line ≠ current stock source (dodge:187, rollSkill:194) — expected (logging line shifts); the mechanic (reroll re-entry) is what matters. Fix must be in the Rust dodge/reroll path or the Rust team-build skill/prayer application; add a test; VERIFY seed 46 advances AND seeds 1-45 hold.

## Iter 46 — seed 46 SOLVED: armour roll must use modified AV (Dodgy Snack -AV)
- **Resolution of the Iter45 frontier.** The half-2 kickoff rolled the BB2025 event **Dodgy Snack** (`StepApplyKickoffResult.handleDodgySnack` at DICE pos44-46) which picks a random player per lower-roll team and, on the snack d6 ≠ 1, applies a `DODGY_SNACK` enhancement = **-MA and -AV for the drive**. Here home_02 was targeted (roll_home=2, roll_away=3 → home player; snack roll=4 → enhancement) → AV 8→7.
- **The "double dodge die" was a misread:** pos47 = home_01's dodge (i=175, success), pos48 = home_02's dodge (i=176, fail) — two different players, one die each, NOT a reroll. The real divergence was downstream: home_02's armour roll = 2+5 = **7**. BB2025 breaks armour on `roll >= AV`. Java uses `DiceInterpreter.isArmourBroken → defender.getArmourWithModifiers()` = AV7 → 7>=7 **breaks** → injury → **Stunned**. Rust's `do_armor_roll`/`recalc_armor_broken` (injury.rs) used base `p.armour` = AV8 → 7<8 **holds** → **Prone**. The -AV enhancement WAS applied to the player (Rust's handle_dodgy_snack adds the temporary_stat_mod correctly) — it just wasn't read at the armour roll.
- **Fix:** `injury.rs` `do_armor_roll` + `recalc_armor_broken` now use `p.armour_with_modifiers()` instead of `p.armour`, matching Java's `getArmourWithModifiers()`.
- **Test:** `armor_roll_uses_modified_armour_not_base` (injury.rs).
- **Result:** PARITY 46/46 (seeds 1-46 green). ffb-engine 7006/0. Frontier ≥ 47.
- **NOTE for later:** other armour-break sites still read base `p.armour` — `injury_type_block.rs:43` (recalc_armor_broken_claws_aware), `reroll_armour_modification.rs:46`, `old_pro_modification.rs`, `injury_context_modification.rs:230`. These will diverge if an AV-modified (snacked/prayer/injured) player is BLOCKED/fouled; fix them the same way when a frontier seed hits it.

## Iter 47 (IN PROGRESS) — seed 57 frontier: half-2 TimeOut turn-counter divergence (UNSOLVED)
- **Status:** seeds 1-56 + 58 GREEN (verified individually). Seed 57 = frontier. NOT fixed.
- **Divergence:** seed 57 i=138 — the half-1→half-2 transition. Both match through i=137 (turn 8 half 1, rng=41). At i=138 Java = **turn 1** half 2 (rng 59), Rust = **turn 2** half 2 (rng 67). Rust is ONE TURN AHEAD and consumed **8 extra dice** (an extra turn's worth of activations played before i=138).
- **Root area:** the half-2 kickoff event is **"TimeOut"** (Rust event `kickoffTimeout turn_number:0 turn_modifier:1`; the other half's event was "Charge"). TimeOut adjusts both teams' turn_nr by ±1 (kicking-team turn>=6 → -1 else +1). The Rust `handle_timeout` and Java `handleTimeout` are BYTE-IDENTICAL (kicking_turn, modifier = turn>=6?-1:1, both turn_nr += modifier). So the handler is NOT the bug.
- **Hypothesis:** Rust's `turn_nr` is already 1 higher than Java's when TimeOut fires, OR Rust double-increments turn_nr across the half-2 setup (kickoff → TimeOut(+1) → turn-start increment). NEXT: trace turn_nr through the half-2 transition sequence in both engines (FFB_DRIVE_TRACE + log turn_data_home/away.turn_nr at each step from end-of-half-1 through the first StartTurn of half 2). Find where Rust's turn_nr diverges from Java's — likely a StepStartTurn/StepEndTurn or half-init increment that shouldn't fire (or fires twice) when TimeOut already bumped the counter. Fix the Rust turn-counter lifecycle; add a test; VERIFY seed 57 advances AND seeds 1-56/58 hold.

## Iter 48 — seed 57 SOLVED: implement Sweltering Heat fainting at half break
- **Resolution of Iter47.** The turn-counter divergence was a downstream symptom. Real root cause: at the half-1→half-2 break the weather is **Sweltering Heat**, and Java's `StepEndTurn.getFaintingCount` rolls ONE d3 (fainting count) then, for each team [home, away], selects that many random on-pitch players (`rollDice(onPitch.size())` per faint, list shrinking) → EXHAUSTED + box. Rust instead rolled a **d6 per on-pitch player** (`is_exhausted` check) — wrong mechanic + wrong dice. First divergent die: pos 43 (Java d3 vs Rust d6). This desynced the whole stream, so the half-2 kickoff-result 2d6 landed on different values → Rust got "TimeOut" (turn +1 → extra turn) where Java rolled 6+2=8 = "Weather Change".
