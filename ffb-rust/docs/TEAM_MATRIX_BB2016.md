...
- ITER91 (vampire 39, UNCHANGED): Blood Lust now rolls once per ACTIVATION (Java uses the ActingPlayer's used-set, cleared by changeActingPlayer) instead of once per game. Correct 1:1 port + test, but moved no seed. Frontier: Rust's seed-1 game ends at i=101 vs Java's 187, right after the Blood Lust failure.
- ITER92 (vampire 39, UNCHANGED, no fix): the seed-1 truncation is a Rust STALL — after the declined Blood Lust re-roll the engine returns `prompt_after=None, finished=false`. Both engines prompt and decline identically (Java's bb2016 RollMechanic does show the dialog; ParityRunner declines with 0 dice). Suspect: a `GotoLabel` returned from `handle_command` is not carried through by the driver the way one returned from `start` is.
- ITER93 (vampire 39, UNCHANGED, no fix): ITER92's driver-asymmetry hypothesis was WRONG — both dispatchers handle GotoLabel identically. A probe for `Continue` with no prompt names the stall: a SECOND block sequence is pushed after the Blood Lust GOTO, and its `StepInitBlocking` has no `block_defender_id` so it waits forever for a CLIENT_BLOCK. Next: find what re-pushes that sequence.
- ITER94 (vampire 39, UNCHANGED, no fix): Java re-pushes the block sequence on a failed Blood Lust too, and Rust's `step_end_blocking.rs:263` already ports that branch faithfully. The gap is downstream — Rust's no-defender `StepInitBlocking` returns `cont()` with NO prompt, so the agent is never asked for a block target the way ParityRunner's phase-2 `sendBlockAction` supplies one. ITER95: add that prompt + the matching agent answer.
- ITER95 (vampire **39 -> 30**): new `AgentPrompt::BlockTarget` — `StepInitBlocking` with no defender now ASKS the agent (Java waits for CLIENT_BLOCK and ParityRunner's sendBlockAction supplies one) instead of returning a promptless `cont()` that stalled the driver.
