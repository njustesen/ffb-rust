package com.fumbbl.ffb.server.skill;

import com.fumbbl.ffb.RulesCollection;
import com.fumbbl.ffb.SkillCategory;
import com.fumbbl.ffb.model.property.NamedProperties;
import com.fumbbl.ffb.skill.bb2016.NoHands;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;

class NoHandsSkillTest {

    private NoHands skill;

    @BeforeEach
    void setUp() {
        skill = new NoHands();
        skill.postConstruct();
    }

    @Test
    void name_is_No_Hands() {
        assertEquals("No Hands", skill.getName());
    }

    @Test
    void category_is_extraordinary() {
        assertEquals(SkillCategory.EXTRAORDINARY, skill.getCategory());
    }

    @Test
    void has_prevent_catch_property() {
        assertTrue(skill.hasSkillProperty(NamedProperties.preventCatch));
    }

    @Test
    void is_bb2016_edition() {
        RulesCollection annotation = NoHands.class.getAnnotation(RulesCollection.class);
        assertNotNull(annotation);
        assertEquals(RulesCollection.Rules.BB2016, annotation.value());
    }
}
