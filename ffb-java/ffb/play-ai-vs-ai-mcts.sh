#!/bin/bash
# Start a local MCTS-AI vs MCTS-AI Blood Bowl game (both sides use MCTS).
# Usage: ./play-ai-vs-ai-mcts.sh [--mcts-budget N]
#   --mcts-budget N  MCTS rollout iterations per activation (default: 10)
#
# Both agents run headlessly. No GUI windows open.
...
# ── 9. Kill any stale AI agents ──────────────────────────────────────────────

pkill -f "com.fumbbl.ffb.ai.AiMain" 2>/dev/null || true
sleep 1

# ── 10. Launch both MCTS AI agents ───────────────────────────────────────────

echo ">> Launching MCTS AI agent 1 (Kalimar, budget=$MCTS_BUDGET)..."
cd "$CLIENT_DIST" && java -cp "$AI_CP" com.fumbbl.ffb.ai.AiMain \
  -coach Kalimar -password test -server localhost -port 22227 -home \
  -teamId teamHumanKalimar -teamName "Kalimar's Humans" \
  -mcts-budget "$MCTS_BUDGET" \
  > /tmp/ffb-ai-kalimar-mcts.log 2>&1 &
AI1_PID=$!

sleep 1

echo ">> Launching MCTS AI agent 2 (BattleLore, budget=$MCTS_BUDGET)..."
cd "$CLIENT_DIST" && java -cp "$AI_CP" com.fumbbl.ffb.ai.AiMain \
  -coach BattleLore -password test -server localhost -port 22227 \
  -teamId teamHumanBattleLore -teamName "BattleLore's Humans" \
  -mcts-budget "$MCTS_BUDGET" \
  > /tmp/ffb-ai-battlelore-mcts.log 2>&1 &
AI2_PID=$!

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  Both MCTS AI agents are running headlessly (budget=$MCTS_BUDGET)."
echo ""
echo "  AI 1 (Kalimar) log:     /tmp/ffb-ai-kalimar-mcts.log"
echo "  AI 2 (BattleLore) log:  /tmp/ffb-ai-battlelore-mcts.log"
